...
const AttendingEventList = ({ navigation }) => {
    ...
    {/*filters the attending events*/ }
    const attendingEvents = events.filter((event) => event.isAttending);

    const toggleAttendStatus = (eventId) => {
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Attending Events</Text>
            {/*flatlist that renders only attending events*/}
            {attendingEvents.length > 0 ? (
                <FlatList
                    data={attendingEvents}
                    keyExtractor={(item) => item.id.toString()}
                    renderItem={({ item }) =>
                        <EventCard event={item} navigation={navigation} onToggleAttend={() => toggleAttendStatus(item.id)} />}
