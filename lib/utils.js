...
const fetchEvents = async (isCreator, id) => {
  ...
  if (isCreator) {
    // if the user is a creator, fetch only the events created by the user
...
const handleEventList = async (setEvents, isCreator) => {
  ...
  const eventsData = await fetchEvents(isCreator, user.id)
