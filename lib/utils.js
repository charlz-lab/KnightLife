...
const subscribeToEvents = (setEvents) => {
  const subscription = supabase
    ...
    // .on(
    //   "postgres_changes",
    //   { event: "*", schema: "public", table: "event_status" },
    //   async (payload) => {
    //     // Handle new event status insertion, update, or deletion
...
const subscribeToAnnouncements = (setAnnouncements) => {
  const subscription = supabase
    .channel("announcement_changes")
    // listen for changes in the events table
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "events_updates" },
      (payload) => {
        if (payload.eventType === "INSERT") {
          // Handle new event insertion
